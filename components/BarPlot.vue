    ...
    <v-row class="mt-4" id="todownload" :class="{ 'centered-download': isDownloading }">
      ...
      <div class="col-4" v-if="sorted">
        <transition name="fade">
          <v-simple-table class="agents-table" height="800px" fixed-header 
...
export default {
  ...
  computed: {
    quartileDataArray() {
      ...
      const array = Object.entries(this.quartileData).map(([agent, quartile]) => ({ agent, quartile }));
      // Sort the array alphabetically
      return array.sort((a, b) => a.agent.localeCompare(b.agent));
    }
  },
  methods: {
    ...
    async renderChart() {
      ...
      const x = data.challenge_participants.map(entry => entry.agent_id);
    ...
    // ----------------------------------------------------------------
    animateBars(data) {
      const x = data.map(entry => entry.agent_id);
    ...
    addLinesBetweenQuartiles() {
      ...
      const agents = Object.keys(this.quartileData);

      // Iterate over the agents to find transitions between quartiles
      for (let i = 1; i < agents.length; i++) {
        const currentAgent = this.quartileData[agents[i]];
        const previousAgent = this.quartileData[agents[i - 1]];

        // If the quartile of the current agent is different from the previous agent, draw a line between them
        if (currentAgent.quartile !== previousAgent.quartile) {
          // Calculate the x-position for the line between the current and previous agents
    ...
    addQuartileLabels() {
      ...
      let uniqueQuartiles = []; // Array to store quartiles with only one agent

      // Count the occurrences of each quartile number
      agents.forEach(agent => {
        const quartile = this.quartileData[agent].quartile;
        quartileCounts[quartile] = (quartileCounts[quartile] || 0) + 1;
      });

      // Identify quartiles with only one agent
      ...
      // Iterate over the agents to add quartile labels
      agents.forEach(agent => {
        const quartile = this.quartileData[agent].quartile;
        ...
        if (quartileCounts[quartile] === 1) {
          // If quartile occurs only once, place the label above the agent
          labelPosition = agents.indexOf(agent);
        } else {
          // If quartile occurs multiple times, calculate the midpoint between agents with the same quartile
          const positions = agents.reduce((acc, curr, index) => {
    ...
    calculateQuartiles(data) {
      ...
      data.forEach(entry => {
        ...
        if (this.datasetPolarity === "minimum") {
          if (metricValue <= q1) {
            metricPositions[entry.agent_id] = { quartile: 1, bgColor: 'rgb(237, 248, 233)' };
          } else if (metricValue > q1 && metricValue <= q2) {
            metricPositions[entry.agent_id] = { quartile: 2, bgColor: 'rgb(186, 228, 179)' };
          } else if (metricValue > q2 && metricValue < q3) {
            metricPositions[entry.agent_id] = { quartile: 3, bgColor: 'rgb(116, 196, 118)' };
          } else if (metricValue >= q3) {
            metricPositions[entry.agent_id] = { quartile: 4, bgColor: 'rgb(35, 139, 69)' };
          }
        } else {
          if (metricValue <= q1) {
            metricPositions[entry.agent_id] = { quartile: 4, bgColor: 'rgb(35, 139, 69)' };
      ...
      try {
        ...
        if (format === 'pdf') {
          ...
          if (this.sortOrder === 'sorted' && Object.keys(this.quartileData).length > 1) {
            ...
            const rows = this.quartileDataArray.map(q => [q.agent, q.quartile.quartile]);
