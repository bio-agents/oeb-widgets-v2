    ...
    <v-row id="todownload" :class="{ 'centered-download': isDownloading }">
      ...
      <div class="col-4" v-if="sorted">
        <transition name="fade">
          <v-simple-table class="agents-table" height="800px" fixed-header v-if="tableData.length > 0" id="benchmarkingTable">
            <thead>
              <tr>
                <th class="agents-th">Participants</th>
                <th class="classify-th">{{ viewKmeans ? 'Clusters' : 'Quartile' }}
                  <v-agenttip :key="viewSquare" bottom>
                    <template v-slot:activator="{ on, attrs }">
                      <i
            ...
            <tbody>
              <tr v-for="(item, index) in tableData" :key="item.agent_id"
                    :class="{ 'quartil-zero': item.cuartil === 0 }">
                <td class="agentColumn" @click="handleTableRowClick(index)">
                  <div class="color-box"
...
export default {
  ...
  methods: {
    ...
    async renderChart() {
      ...
      this.agentID = data.challenge_participants.map((participant) => participant.agent_id);
      this.allAgentID = data.challenge_participants.map((participant) => participant.agent_id);
    ...
    updatePlotOnSelection(traceIndex) {
      ...
      const agentHidden = this.dataPoints[traceIndex].hidden;

      if (!agentHidden) {
        const visibleAgents = this.dataPoints.filter((agent) => !agent.hidden);
        if (visibleAgents.length <= 4) {
      ...
      this.dataPoints[traceIndex].hidden = !agentHidden;

      const updatedVisibleAgents = this.dataPoints.filter((agent) => !agent.hidden);

      const newTraces = null;
      if (this.optimalview != null) {
        let direction = this.formatOptimalDisplay(this.optimalview);
        const newParetoPoints = pf.getParetoFrontier(updatedVisibleAgents, { optimize: direction });
      ...
      if (this.viewSquare === true) {
        ...
        const updatedXCoordinates = updatedVisibleAgents.map((participant) => participant[0]);
        const updatedYCoordinates = updatedVisibleAgents.map((participant) => participant[1]);

        // Create a list of visible agents with their hiding status
        const visibleAgents = this.agentID.map((agent, index) => ({
          name: agent,
          hidden: this.dataPoints[index].hidden
        })).filter(agent => !agent.hidden);

        // List of visible agents
        const visibleAgentNames = visibleAgents.map(agent => agent.name);
        // Update data with visible agents
        this.calculateQuartiles(updatedXCoordinates, updatedYCoordinates, visibleAgentNames);
        this.optimalView()
      }
      ...
      if (this.viewKmeans === true) {
          ...
          this.createShapeClustering(updatedVisibleAgents, visibleAgentNames, better, this.allAgentID);
    ...
    toggleQuartilesVisibility () {
      ...
      if (plot && plot.data) {
        ...
        this.calculateQuartiles(this.xValues, this.yValues, this.agentID);
        this.optimalView();
    ...
    calculateQuartiles (xValues, yValues, agentID){

      const cuartilesX = statistics.quantile(xValues, 0.5);
      const cuartilesY = statistics.quantile(yValues, 0.5);

      let better = this.optimalview
      let allAgentsWithId = this.allAgentID
      this.sortAgentsForSquare(better, allAgentsWithId, agentID, cuartilesX, cuartilesY, xValues, yValues)
    ...
    // Sort agents for Square Quartiles
    sortAgentsForSquare(better, allAgentsWithId, visibleAgentID, cuartilesX, cuartilesY, xValues, yValues) {
      this.tableData = [];
      allAgentsWithId.forEach((agent) => { // Iterate over all agents
        const index = visibleAgentID.indexOf(agent);
        ...
        if (index !== -1) { // Si la herramienta está presente en visibleAgentID
        ...
        this.tableData.push({ agent_id: agent, cuartil: cuartil, label: label });
      });
    },
    ...
    let agents_not_hidden = x_values.map((x, i) => [x, y_values[i]]);
    ...
    // Get Annotations
    let annotationDiagonal = this.asigneQuartileDiagonal(agents_not_hidden, first_quartile, second_quartile, third_quartile,better)

    // Diagonal Q. Table
    this.createTableDiagonal(agents_not_hidden)
    ...
    getDiagonalline (scores, scores_coords, quartile, better, max_x, max_y) {
      ...
      for(let i = 0; i < scores.length; i++){
        if(scores[i] <= quartile){
          // When la longitud de las agents es menor de la esperada se ejecuta este condicional.
          if (i == 0){i = 1}
    ...
    asigneQuartileDiagonal (dataAgents, first_quartile, second_quartile, third_quartile, better) {
      
      let poly = [[],[],[],[]];
      dataAgents.forEach(element => {
        if (better == 'top-left'){
    ...
    // Create Table
    createTableDiagonal(visibleAgent) {
      this.tableData = [];

      this.allAgentID.forEach((agent) => {
        const agentName = agent;
        const visibleAgentInfo = visibleAgent.find(item => item[0] === this.xValues[this.allAgentID.indexOf(agent)]);

        let quartile = 0;
        let label = '--';

        if (visibleAgentInfo) {
          quartile = visibleAgentInfo.quartile;
          label = quartile.toString();
        }

        this.tableData.push({ agent_id: agentName, cuartil: quartile, label: label });
      });
    },
    ...
    toggleKmeansVisibility () {
      ...
      if (plot && plot.data) {
        ...
        let better = this.optimalview
        this.createShapeClustering(this.dataPoints, this.agentID, better, this.allAgentID);
    ...
    createShapeClustering (dataPoints, agentIDVisible, better, allAgentID) {
      clusterMaker.k(4);
      ...
      this.createDataPointForTables(agentIDVisible, groupedDataPoints, allAgentID)
    ...
    createDataPointForTables (visibleAgents, groupedDataPoints, allAgentID) {
      this.tableData = [];
      allAgentID.forEach((agent) => {
        const index = visibleAgents.indexOf(agent);
        let cuartil = 0;
        let label = '--';
        if (index !== -1) {
...
.agents-table .agents-th {
  width: 60%;
  /* border-top-left-radius: 10px; */
}

.agents-table .classify-th {
  width: 40%;
  /* border-top-right-radius: 10px; */
}
